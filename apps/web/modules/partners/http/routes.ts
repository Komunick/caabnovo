...
interface Dependencies {
  ...
  service: {
    benefits(actor: RequestActor, input: unknown): Promise<unknown>;
    list(actor: RequestActor, input: unknown): Promise<unknown>;
    get(actor: RequestActor, id: string): Promise<unknown>;
    create(context: PartnerContext, input: unknown): Promise<unknown>;
    command(context: PartnerContext, id: string, input: unknown): Promise<unknown>;
...
export function createPartnerRoute(deps: Dependencies) {
  return async (request: Request, path: string[] = []): Promise<Response> => {
    ...
    try {
      ...
      if (request.method === "GET") {
        ...
        } else if (id && path.length === 2 && path[1] === "commands") {
          const input = partnerCommandSchema.parse(await readJson(request));
          requirePermission(
            actor,
            ["contract-status", "publish", "hide"].includes(input.action)
