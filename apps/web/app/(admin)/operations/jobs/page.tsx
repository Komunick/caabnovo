...
export default async function JobsPage() {
  ...
  const actor = await resolveRequestActor(
    new Request("http://caab.internal/operations/jobs", { headers: requestHeaders }),
  );
  if (!actor?.permissions.has(PERMISSIONS.jobsRead)) {
    return <p role="alert">Você não tem permissão para acessar operações.</p>;
  }
            ...
            <Table caption="Processamentos recentes">
              ...
              <tbody>
                {jobs.map((job) => (
                  <tr key={job.id}>
                    <th scope="row">
                      <Link href={`/operations/jobs/${job.id}`}>{job.jobType}</Link>
                    </th>
                    <td>
