...
export async function importLegacyFile(pool: Pool, id: string, originalKey: string, body: Buffer) {
  return withTransaction(pool, async (client) => {
    const result = await client.query<{
      checksum_sha256: string | null;
      size_bytes: string | null;
    }>(
      `SELECT checksum_sha256,size_bytes::text FROM stored_file WHERE id=$1 AND object_key=$2
       AND status='available' AND scan_result='clean' AND deleted_at IS NULL FOR UPDATE`,
      [id, originalKey],
    );
    const file = result.rows[0];
    if (!file) throw fileContentError("LEGACY_FILE_CHANGED");
    if (
      file.size_bytes === null ||
      Number(file.size_bytes) !== body.length ||
      !file.checksum_sha256 ||
      createHash("sha256").update(body).digest("hex") !== file.checksum_sha256
    )
      throw fileContentError("LEGACY_INTEGRITY_MISMATCH");
    const key = `database/private/${id}`;
    await client.query(
      "INSERT INTO stored_file_content(file_id,object_key,body) VALUES($1,$2,$3)",
      [id, key, body],
    );
    const verified = await client.query(
      `SELECT 1 FROM stored_file_content WHERE file_id=$1
       AND octet_length(body)=$2 AND encode(sha256(body),'hex')=$3`,
      [id, body.length, file.checksum_sha256],
    );
    if (!verified.rowCount) throw fileContentError("DATABASE_INTEGRITY_MISMATCH");
    await client.query("UPDATE stored_file SET object_key=$2,updated_at=now() WHERE id=$1", [
      id,
      key,
    ]);
  });
