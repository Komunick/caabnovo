-- pg-boss 12.30.0 schema version 40.
-- Queue objects are isolated from the application schema and owned by the runtime role so
-- pg-boss can perform its own queue and partition maintenance without broader DDL privileges.
CREATE SCHEMA IF NOT EXISTS pgboss AUTHORIZATION caab_runtime;
SET LOCAL ROLE caab_runtime;
...
CREATE TABLE pgboss.version (
  ...
  bam_on timestamptz,
  flow_on timestamptz,
  reindex_on timestamptz,
  monitor_backoff_on timestamptz
);
...
CREATE FUNCTION pgboss.job_table_run(
  command text,
...
BEGIN
  ...
  FOR tbl IN SELECT table_name FROM pgboss.queue WHERE partition = true LOOP
    EXECUTE pgboss.job_table_format(command, tbl.table_name);
  END LOOP;
END;
$$ LANGUAGE plpgsql;

CREATE FUNCTION pgboss.job_table_run_async(
  command_name text,
  version int,
  command text,
  tbl_name text DEFAULT NULL,
...
SELECT pgboss.job_table_run(
  'ALTER TABLE pgboss.job ADD PRIMARY KEY (name, id)',
  'job_common'
);
SELECT pgboss.job_table_run(
  'ALTER TABLE pgboss.job ADD CONSTRAINT q_fkey FOREIGN KEY (name) REFERENCES pgboss.queue (name) ON DELETE RESTRICT DEFERRABLE INITIALLY DEFERRED',
  'job_common'
);
SELECT pgboss.job_table_run(
  'ALTER TABLE pgboss.job ADD CONSTRAINT dlq_fkey FOREIGN KEY (dead_letter) REFERENCES pgboss.queue (name) ON DELETE RESTRICT DEFERRABLE INITIALLY DEFERRED',
  'job_common'
);
SELECT pgboss.job_table_run(
...
SELECT pgboss.job_table_run(
  'CREATE INDEX job_i10 ON pgboss.job (name, singleton_key, state DESC, created_on, id) INCLUDE (start_after) WHERE state < ''active'' AND NOT blocked AND policy = ''key_strict_fifo''',
  'job_common'
);
SELECT pgboss.job_table_run(
...
SELECT pgboss.job_table_run(
  'CREATE INDEX job_i11 ON pgboss.job (name, priority DESC, created_on, start_after) WHERE state < ''active'' AND NOT blocked',
  'job_common'
);
SELECT pgboss.job_table_run(
...
CREATE INDEX job_dep_parent_idx ON pgboss.job_dependency (parent_name, parent_id);

CREATE FUNCTION pgboss.create_queue(queue_name text, options jsonb)
RETURNS void AS $$
DECLARE
  tablename varchar := CASE
    WHEN options->>'partition' = 'true' THEN 'j' || encode(sha224(queue_name::bytea), 'hex')
    ELSE 'job_common'
  END;
...
BEGIN
  ...
  EXECUTE pgboss.job_table_format(
    'ALTER TABLE pgboss.job ADD PRIMARY KEY (name, id)',
    tablename
  );
  EXECUTE pgboss.job_table_format(
  ...
  EXECUTE format(
    'ALTER TABLE pgboss.job ATTACH PARTITION pgboss.%I FOR VALUES IN (%L)',
    tablename,
    queue_name
  );
END;
