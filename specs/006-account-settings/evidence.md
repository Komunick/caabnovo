...
- Na primeira correção, a CI por push passou integralmente. A execução por PR aprovou o cenário sem MFA, mas expôs a mesma interpolação de baixo contraste nos botões secundário Notícias e Limpar filtros. A troca simultânea de cores foi então aplicada a todos os botões de Notícias. Uma regressão permanente mede o contraste renderizado quadro a quadro durante as duas trocas de tema, interpretando color-mix e transparência pelo navegador; passou localmente. Nenhuma regra de axe foi desativada e não foram acrescentadas esperas para esconder o estado intermediário.
