...
- Associados reserva `packages/db/migrations/0010_members.sql` e `0013_member_administrative_status.sql`. As migrations 0011 e 0012
...
- Leitura no servidor: `@caab/db/repositories/members`, função `findMemberSummary(client, id)`; retorna `{ id, name, archivedAt, administrativeStatus } | null`. `archivedAt` é string ISO ou null; campo aditivo `administrativeStatus` é inactive, active ou blocked. Ativo não concede crédito; bloqueio administrativo não altera automaticamente saldos ou avaliações financeiras.
...
## Integração futura com Agenda — confirmada em 10/09/2026

Ao implementar a integração, Agenda deve revalidar no servidor o bloqueio do associado e
dos titulares com vínculo de dependência vigente antes de confirmar um novo agendamento.
Bloqueio de qualquer titular vigente impede novos agendamentos do dependente; não altera
o cadastro nem a situação administrativa própria dele. Não confiar em estado enviado pelo
navegador. Tratar concorrência com bloqueio/vínculos durante a confirmação. O resumo por ID
expõe a situação própria, mas não resolve titulares; ampliar o contrato de leitura para essa
consulta na entrega de Agenda. Esta entrega não declara Agenda já protegida ou integrada.

Desbloqueio é manual; não restaurar agendamentos cancelados nem cancelar os existentes
automaticamente. Política para agendamentos anteriores e outras finalidades precisa ser
definida na spec de Agenda. Não copiar prazo automático de 30 dias do legado (LEG-002).
