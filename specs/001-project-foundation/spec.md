...
Escopo reduzido por pedido do usuário para preservar compatibilidade com a VM. Manter somente bloqueio de inscrição pública, criação administrativa existente e atualizações de dependências. Retirar desta entrega migration 0024, alteração de permissões, CSP/cabeçalhos/cache/renderização, restrições de seeds, validação adicional de credenciais e mudanças de portas. Não alterar banco, configuração ou contas da VM. GitHub settings/workflows continuam excluídos.
